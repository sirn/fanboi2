...
class TestJSONRenderer(_RendererMixin, unittest.TestCase):
    ...
    def test_result_proxy_success(self):
        ...
        result_proxy = ResultProxy(DummyAsyncResult(
            'demo',
            'success',
            result))
...
class TestJSONRendererWithModel(
        _RendererMixin,
        ModelSessionMixin,
        unittest.TestCase):
    ...
    def setUp(self):
        ...
        from . import mock_service
        from ..interfaces import ISettingQueryService
        self.request = mock_service(self.request, {
            ISettingQueryService: _DummySettingQueryService()})
    ...
    def test_query(self):
        ...
        response = self._make_one(
            self.request,
            self.dbsession.query(Board).order_by(Board.title))
