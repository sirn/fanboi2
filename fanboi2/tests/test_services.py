...
class TestRuleBanCreateService(ModelSessionMixin, unittest.TestCase):
    ...
    def test_create(self):
        ...
        self.assertGreaterEqual(
            rule_ban.active_until, datetime.now() + timedelta(days=29, hours=23)
        )
...
class TestRuleBanQueryService(ModelSessionMixin, unittest.TestCase):
    ...
    def test_is_banned(self):
        ...
        self.assertTrue(rule_ban_query_svc.is_banned("10.0.1.1", scopes=["foo:bar"]))
        self.assertTrue(rule_ban_query_svc.is_banned("10.0.1.255", scopes=["foo:bar"]))
    ...
    def test_is_banned_scoped(self):
        ...
        self.dbsession.commit()
        rule_ban_query_svc = self._get_target_class()(self.dbsession)
        self.assertTrue(rule_ban_query_svc.is_banned("10.0.4.1", scopes=["foo:bar"]))
        self.assertTrue(rule_ban_query_svc.is_banned("10.0.4.255", scopes=["foo:bar"]))
    ...
    def test_rule_ban_from_id(self):
        ...
        self.dbsession.commit()
        rule_ban_query_svc = self._get_target_class()(self.dbsession)
        self.assertEqual(rule_ban_query_svc.rule_ban_from_id(rule_ban.id), rule_ban)
    ...
    def test_rule_ban_from_id_string(self):
        ...
        rule_ban_query_svc = self._get_target_class()(self.dbsession)
        self.assertEqual(
            rule_ban_query_svc.rule_ban_from_id(str(rule_ban.id)), rule_ban
        )
...
class TestRuleBanUpdateService(ModelSessionMixin, unittest.TestCase):
    ...
    def test_update_none(self):
        ...
        rule_ban = self._make(
            RuleBan(ip_address="10.0.1.0/24", description="In a galaxy far away")
        )
        self.dbsession.commit()
        rule_ban_update_svc = self._get_target_class()(self.dbsession)
    ...
    def test_update_empty(self):
        ...
        rule_ban_update_svc = self._get_target_class()(self.dbsession)
        rule_ban = rule_ban_update_svc.update(
            rule_ban.id, description="", duration="", scope="", active=""
        )
        self.assertIsNone(rule_ban.description)
        self.assertIsNone(rule_ban.active_until)
        self.assertIsNone(rule_ban.scope)
